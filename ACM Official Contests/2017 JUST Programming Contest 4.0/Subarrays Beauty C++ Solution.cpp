	...
	for (int tt = 0; tt < t; tt ++){
		...
		for (int i = (n - 1); i >= 0; i --){
			...
			for (int ii = 0; ii < 23; ii ++){
				if (b[ii] == 0) near[i][ii] = i;
				else near[i][ii] = near[i + 1][ii];
			}
		}
		...
		for (int i = 0; i < n; i ++){
			...
			while (ii < n){
				...
				for (int x = 0; x < 23; x ++)
					if (b[x]) minima = min(minima, near[ii][x]);

				if (minima >= 1e8){
					res += (ll)(b.to_ulong() * (ll)(n - ii));
					break;
				}
				else {
					res += (ll)(b.to_ulong() * (ll)(minima - ii));
					ii = minima;
				}
			}
		}
