...
int main() {
	...
	for (int tt = 0; tt < t; tt ++){
		ll n, q, l, r; cin >> n >> q; mm(cw, 0);
		char target; string str; cin >> str;

		for (int i = 0; i < n; i ++){
			for (char ii = 'a'; ii <= 'z'; ii ++){
				if (str[i] == ii) cw[i][ii - 'a'] = 1;
				if (i > 0) cw[i][ii - 'a'] += cw[i - 1][ii - 'a'];
			} 
		}

		for (int i = 0; i < q; i ++){
			cin >> l >> r >> target; l--; r--;
			ll ln = (l - r) + 1, res = 0, L = (l % n), R = (r % n);

			if ((l + ((n - 1) - L)) < r){ 
				if (L == 0) res = cw[n - 1][target - 'a'];
				else res = cw[n - 1][target - 'a'] - cw[L - 1][target - 'a'];
				l += ((((n - 1) - L)) + 1);
				res += (((((r - l) + 1) / n) * cw[n - 1][target - 'a']));
				if ((((r - l) + 1) % n) > 0) res += (cw[(((r - l) + 1) % n) - 1][target - 'a']);
			}
			else {
				if (L == 0) res = cw[R][target - 'a'];
				else res = cw[R][target - 'a'] - cw[L - 1][target - 'a'];
			}
