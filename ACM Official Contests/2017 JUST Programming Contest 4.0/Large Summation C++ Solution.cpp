...
int main() {
	...
	for (int tt = 0; tt < t; tt ++){
		...
		for (int i = 0; i < n; i ++)
			scanf("%lld", &seq[i].fi), seq[i].se = i; 

		sort(ALL(seq));

		for (int i = 0; i < n; i ++){
			ll maxima = -1e10, target = mod - seq[i].fi, start = 0, end = (n - 1), mid;

			while (abs(end - start) > 50){
				mid = start + (end - start) / 2;
				if (seq[mid].fi <= target) start = mid;
				else end = mid - 1;
			}

			for (int x = start; x <= end; x ++){
				if (x != i && seq[x].fi <= target) 
					maxima = max(maxima, ((seq[x].fi + seq[i].fi) % (mod + (ll)1)));
			}

			for (int x = 0; x < min((ll)5, n); x ++){
				if (x != i) 
					maxima = max(maxima, ((seq[x].fi + seq[i].fi) % (mod + (ll)1)));	
			}

			for (int x = (n - 1), cw = 0; (x >= 0 && cw < 5); x --, cw ++){
				if (x != i) 
					maxima = max(maxima, ((seq[x].fi + seq[i].fi) % (mod + (ll)1)));
			}
