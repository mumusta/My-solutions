...
void solve(){
    ...
    for (int i = 1; i <= 2000; i ++){
        res[i] = res[i - 1];
        if (res[i].back() != 0) res[i].pp(0);
        int who, where;
        for (int ii = 1; ii <= res[i].size(); ii ++){
            int in = ii - 1;
            if (res[i][in] == 0) { who = ii; where = in; break; }
        }
        for (int ii = 0 ; ii < where; ii ++)
            res[i][ii]--; res[i][where] = who;
    }
    ...
    for (int tt = 0 ; tt < t; tt ++){
        ...
        for (int i = 0 ; i < res[n].size(); i ++){
            ...
            if (cw == 10) {
                ...
                if (i != (res[n].size() - 1)) cout << " ";
            }
        }
        if (res[n].size() % 10 != 0) cout << endl;
    }
