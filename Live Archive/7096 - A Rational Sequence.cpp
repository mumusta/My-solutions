...
void goLeft(int &p, int &q) { q += p; }
void goRight(int &p, int &q) { p += q; }
void undo(int &p, int &q){ if (p < q) q -= p; else p -= q; }
...
int main() {
   // ______________________________________________________________________
    ...
    int t, k, p, q; char c; cin >> t;

    for (int tt = 0 ; tt < t; tt ++){
        cin >> k >> p >> c >> q;
        if (p < q){
            undo(p, q); goRight(p, q);
        }
        else {
            if (q == 1) {
                swap(p, q); goLeft(p, q);
